...
detect_cryptroot_line() {
    grep -E '^[^#].+' /etc/crypttab 2>/dev/null | head -n 1 || true
}
...
maybe_write_dropbear_ip_config() {
    ...
    local ip_line
    ip_line="$(ask_input "Enter full IP value for /etc/initramfs-tools/initramfs.conf" "")"

    printf 'IP=%s\n' "$ip_line" > /etc/initramfs-tools/initramfs.conf.d/dhcp.conf 2>/dev/null || true
